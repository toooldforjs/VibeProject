...
router.get('/', async (req, res) => {
  try {
    ...
    const result = await pool.query(
      `SELECT c.id, c.issue_key, c.user_id, c.body_markdown,
  ...
  try {
    ...
    const result = await pool.query(
      `INSERT INTO issue_comments (issue_key, user_id, body_markdown)
       VALUES ($1, $2, $3)
       RETURNING id, issue_key, user_id, body_markdown,
    ...
    const createdISO = (row.created_at instanceof Date ? row.created_at : new Date(row.created_at)).toISOString();
    res.status(201).json({
      comment: {
        id: row.id,
        issueKey: row.issue_key,
