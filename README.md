...
Если нажать на странице задачи кнопку "Slop!" формируется запрос к GigaChat, где:

- В запрос пользователя вкладывается описание открытой задачи. Если в тексте задачи были ссылки на страницы Confluence, то получается содержимое этих страниц, преобразуется в текст и добавляется к запросу вместо ссылок в тексте.
- В системный промпт добавляется текст промпта из настроек приложения + текст из раздела "Дополнительный контекст проекта" в настройках (либо заполненный текст, либо содержимое страницы Confluence по указанной ссылке).

Ответ GigaChat показывается в модальном окне с возможностью скопировать его (текст или MD) или сохранить в комментариях к задаче.

Все комментарии в базе приложения и не отправляются в Jira.
...
- Base URL (основной URL-адрес Jira).
